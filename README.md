...
- 自动合并作品身份与 MD5 结果，避免同一文件重复出现在不同组
...
- 候选组及组内文件支持点击列标题排序；大小、时长、分辨率、修改时间按真实数值排序，未知值固定置后
- 结果表直接展示识别作品、身份类型、分段、大小、时长、编码、完整 MD5 状态、目录等信息
...
1. Unicode、大小写、繁简体和尾部罗马数字归一化，并清理画质、编码、站点包装等非作品信息。
2. 按强度依次解析日期单集、季集编号、作品编号和普通影视标题，生成可解释的作品身份键。
3. 日期、季集、作品编号、发行年份或分段任一明确冲突时直接隔离，不再用前缀相似度覆盖冲突。
4. 普通标题只允许精确别名、词序调整或逐词轻微拼写差异；不同实质词语不会因共享系列名而合并。
5. MD5 智能模式对同大小文件先计算三段快速指纹，只对指纹相同者读取完整内容；完整模式则校验全部同大小候选。
6. 合并两条识别通道，对连锁匹配再次按锚点拆组。
7. 读取到视频时长后，为超过安全范围的片长差异组增加保护，不自动建议删除。
